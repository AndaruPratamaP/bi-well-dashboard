    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* AI MCU Upload Button (Prominent Action) */}
          <button
            onClick={() => setIsUploadModalOpen(true)}
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold text-white bg-bi-900 hover:bg-bi-800 rounded-xl shadow-sm transition-all hover:shadow-md hover:scale-[1.01] shrink-0 self-start sm:self-auto"
          >
        ...
        <div className="pt-3.5 border-t border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-4">
            {/* Year Filter */}
            <div className="flex items-center gap-2">
              <span className="font-semibold text-slate-600">Tahun MCU:</span>
              <select
                value={selectedYear}
                onChange={e => onYearChange(Number(e.target.value))}
                className="font-semibold px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-slate-800 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-bi-900"
              >
                {activeYears.map(yr => (
          ...
          <div className="text-slate-400 text-[11px] font-medium flex items-center gap-1.5">
            ...
            <span>Menampilkan <strong>{totalEmployees}</strong> pegawai</span>
          </div>
        </div>
      </div>
